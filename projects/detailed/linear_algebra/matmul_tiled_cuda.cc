...
#include "linear_algebra/matmul_tiled_device.h"
...
std::vector<float> matmul_tiled_cuda(const std::vector<float> &a,
  ...
  check_shape(a, b, c, m, n, k, "matmul_tiled_cuda: size mismatch");
  ...
  launch_matmul_tiled_kernel(d_a.get(), d_b.get(), d_c.get(), d_out.get(), m, n,
