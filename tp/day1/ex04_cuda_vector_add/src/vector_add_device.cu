...
__global__ void vector_add_kernel(const float *a, const float *b, float *out,
                                  float alpha, int n) {
  // TODO: Compute the global element index, guard it with n, and write
  // out[i] = alpha * a[i] + b[i].
...
void launch_vector_add_kernel(const float *a, const float *b, float *out,
  ...
  void *args[] = {const_cast<float **>(&a), const_cast<float **>(&b), &out, &alpha, &n};
  TP_CUDA_CHECK(cudaLaunchKernel(reinterpret_cast<const void *>(&vector_add_kernel), grid,
                                 block, args, 0, stream));
}
