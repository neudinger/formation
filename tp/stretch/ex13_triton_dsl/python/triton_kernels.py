...
def _vector_add_kernel(a_ptr, b_ptr, out_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    ...
    # TODO: Use pid, BLOCK_SIZE, and tl.arange to compute this program's offsets.
    offsets = tl.arange(0, BLOCK_SIZE)
    # TODO: Mask offsets that are outside n_elements.
...
def _saxpy_kernel(a_ptr, b_ptr, out_ptr, alpha, n_elements, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(axis=0)
    # TODO: Compute block-relative offsets for this program instance.
    offsets = tl.arange(0, BLOCK_SIZE)
    # TODO: Protect the final partial block with a mask.
    mask = offsets < n_elements
    a = tl.load(a_ptr + offsets, mask=mask, other=0.0)
    b = tl.load(b_ptr + offsets, mask=mask, other=0.0)
    # TODO: Store alpha * a + b.
    tl.store(out_ptr + offsets, a + b, mask=mask)
...
def _matmul_kernel(
    ...
    # TODO: Iterate over K tiles, load masked A/B tiles, and accumulate tl.dot.
    # The starter deliberately writes zeros so the correctness test fails.
