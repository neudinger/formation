...
struct Buffer {
  ...
  Buffer(std::size_t count, MemoryMode mode) {
    if (mode == MemoryMode::managed) {
      // TODO: Allocate count floats with cudaMallocManaged and check the result.
      throw std::logic_error("TODO: managed allocation");
    } else {
...
std::vector<float> vector_add_cuda(const std::vector<float> &a,
  ...
  if (mode == MemoryMode::managed) {
    // TODO: Copy a and b into the managed input buffers using CPU writes.
    throw std::logic_error("TODO: managed input initialization");
  } else {
  ...
  if (mode == MemoryMode::managed) {
    // TODO: Read the synchronized managed output into result on the CPU.
    throw std::logic_error("TODO: managed result readback");
  } else {
