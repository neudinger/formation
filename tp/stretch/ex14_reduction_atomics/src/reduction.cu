...
__global__ void sum_atomic(const float *input, float *sum, int size) {
  ...
  // TODO: For i < size, add input[i] to sum with atomicAdd.
  (void)i;
  (void)input;
  (void)sum;
  (void)size;
...
__global__ void sum_block(const float *input, float *sum, int size) {
  // TODO: Load 256 values into shared memory; zero-fill inactive lanes.
  // Reduce by halving the stride, with all threads reaching every barrier.
  // Finally, thread 0 performs one atomicAdd for the whole block.
  (void)input;
  (void)sum;
  (void)size;
}
