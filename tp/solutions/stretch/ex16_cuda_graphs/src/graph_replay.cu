...
std::vector<float> repeat_add(const std::vector<float> &input, int batches,
  ...
  if (mode == LaunchMode::graph) {
    CUDA_CHECK(cudaStreamBeginCapture(work.stream, cudaStreamCaptureModeGlobal));
    launch_kernels();
    CUDA_CHECK(cudaStreamEndCapture(work.stream, &work.graph));
    CUDA_CHECK(cudaGraphInstantiate(&work.executable, work.graph, 0));
  }
  auto launch_batch = [&] {
    if (mode == LaunchMode::graph) {
      CUDA_CHECK(cudaGraphLaunch(work.executable, work.stream));
    } else {
      launch_kernels();
    }
  };
